...
PackageConfig.prototype.prompt = function() {
  ...
  .then(function() {
    return ui.input('Enter a %name% for the package (the name your package will be required as)', self.name || 'app');
  })
  .then(function() {
    return ui.input('Enter the %directories.baseURL% public server path (all modules need to be within this folder)', self.directories.baseURL || './');
  })
  .then(function(baseURL) {
    ...
    return ui.input('Enter the local package folder (%directories.lib% containing your package application code)', self.directories.lib || base + 'lib');
  })
  .then(function(lib) {
    self.directories.lib = lib;
    return ui.input('Enter an optional custom %directories.jspmPackages% jspm packages folder (within %directories.baseURL%)', self.directories.packages || base + 'jspm_packages');
  })
  .then(function(packages) {
    self.directories.packages = path.relative(baseDir, path.resolve(packages));
    return ui.input('Enter an optional custom %configFiles.jspm% config file path', self.configFiles.jspm || base + 'config.js');
  })
