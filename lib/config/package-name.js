...
function Package(name) {
  ...
  var version = '';
  var packageString = this.registry ? name.substr(this.registry.length + 1) : name;
  var versionSeparatorIndex = packageString.indexOf('@');

  if (versionSeparatorIndex !== -1) {
      version = packageString.substring(versionSeparatorIndex + 1);      
      this.package = packageString.substring(0, versionSeparatorIndex);
  } else {
