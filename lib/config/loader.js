...
function JspmSystemConfig() {
  var self = this;

  // primary jspm.config.js file
  this.file = new SystemConfigFile(config.pjson.configFile);
  ...
  // location and populate the package.json configFiles.x if it exists in the file system
  ...
  if (!config.pjson.configFileDev) {
    if (this.devFile.timestamp == -1)
      ...
      config.pjson.configFileDev = path.dirname(config.pjson.configFile) + path.sep + 'jspm.dev.js';
  }
  ...
  if (!config.pjson.configFileBrowser) {
    if (this.browserFile.timestamp == -1)
      ...
      config.pjson.configFileBrowser = path.dirname(config.pjson.configFile) + path.sep + 'jspm.browser.js';
  }
  ...
  if (!config.pjson.configFileNode) {
    if (this.nodeFile.timestamp == -1)
      this.nodeFile = null;
    else
      config.pjson.configFileNode = path.dirname(config.pjson.configFile) + path.sep + 'jspm.node.js';
  }

  this.file.changed = false;
  
  // ignore further change events when using separate files for these objects
  this.file.changeEvents.push(function(memberArray) {
    if (['devConfig', 'browserConfig', 'nodeConfig'].indexOf(memberArray[0]) != -1 &&
        self[memberArray[0] + 'File'])
      return true;
  });
  ...
  function readAndSanitizePaths(pathsProp) {
    ...
    if (paths)
      Object.keys(paths).forEach(function(path) {
        ...
        if (self.browserBaseURL && value.startsWith(self.browserBaseURL)) {
          value = value.substr(self.browserBaseURL.length);
          self.file.setValue(pathsProp.concat([path]), value);
        }
        if (path.endsWith('*') && value.endsWith('*') && value[value.length - 2] == '/') {
  ...
  this.browserLibURL = paths[this.packageName + '/'] || browserPaths[this.packageName + '/'];
  this.nodeLibURL = paths[this.packageName + '/'] || nodePaths[this.packageName + '/'];

  if (this.nodeLibURL && !isPlain(this.nodeLibURL)) {
  ...
  if (this.package) {
    if (typeof this.nodeLibURL != 'string')
      this.nodeLibURL = path.relative(config.pjson.baseURL, config.pjson.lib);
...
JspmSystemConfig.prototype.createConfigFile = function(type) {
  this[type + 'File'] = new SystemConfigFile(config.pjson['configFile' + type[0].toUpperCase() + type.substr(1)] || 
      path.dirname(config.pjson.configFile) + path.sep + 'jspm.' + type + '.js');
};
JspmSystemConfig.prototype.ensureRegistry = function(registryName) {
  ...
  if (this.nodeRegistryPaths && !this.nodeRegistryPaths[registryName + ':'])
    this.nodeRegistryPaths[registryName + ':'] = this.nodePackagesURL + '/' + registryName + '/';
};
...
Object.defineProperty(JspmSystemConfig.prototype, 'nodePackagesURL', {
  get: function() {
    this._defaultNodePackagesURL = this._defaultNodePackagesURL || path.relative(config.pjson.baseURL, config.pjson.packages);
    return getPackagesURL(this.nodeRegistryPaths) || this._defaultNodePackagesURL;
  },
  set: function(packagesURL) {
    var self = this;
    ...
    Object.keys(this.nodeRegistryPaths).forEach(function(path) {
      ...
      if (registryPath && registryPath.packagesURL == curPackagesURL)
        self.nodeRegistryPaths[path] = packagesURL + registryPath.name + '/';
    });
...
JspmSystemConfig.prototype.syncFile = function() {
  ...
  if (this.browserBaseURL) {
    if (!this.file.has(['browserConfig', 'baseURL']) && this.file.has(['baseURL']))
      this.file.setValue(['baseURL'], this.browserBaseURL.substr(0, this.browserbaseURL.length - 1));
    else
      this.file.setValue(['browserConfig', 'baseURL'], this.browserBaseURL.substr(0, this.browserBaseURL.length - 1));
  }
  ...
  function setPathsConfig(name, browserPath, nodePath, orderLast) {
    if (typeof nodePath == 'string' && typeof browserPath == 'string' && isPlain(browserPath) && browserPath == nodePath) {
      ...
      self.file.remove(['browserConfig', 'paths', name]);
      self.file.remove(['nodeConfig', 'paths', name]);
      ...
      if (typeof nodePath == 'string' && typeof browserPath == 'string')
        self.file.remove(['paths', name]);
    }
  }

  this.file.clearIfEmpty(['devConfig', 'paths']);
  this.file.clearIfEmpty(['devConfig']);
  this.file.clearIfEmpty(['browserConfig', 'paths']);
  this.file.clearIfEmpty(['browserConfig']);
  this.file.clearIfEmpty(['nodeConfig', 'paths']);
...
JspmSystemConfig.prototype.write = function() {
  ...
  if (config.pjson.configFileDev) {
    if (!this.devFile)
      this.createConfigFile('dev');
    else if (config.pjson.configFileDev != this.devFile.fileName)
  ...
  if (config.pjson.configFileBrowser) {
    if (!this.browserFile)
      this.createConfigFile('browser');
    else if (config.pjson.configFileBrowser != this.devFile.fileName)
  ...
  if (config.pjson.configFileNode) {
    if (!this.nodeFile)
      this.createConfigFile('node');
    else if (config.pjson.configFileNode != this.devFile.fileName)
  ...
  // if we've gone from using an external config to not, then remove
  if (!config.pjson.configFileDev && this.devFile) {
    fs.unlinkSync(this.devFile.fileName);
    this.devFile = null;
  }
  if (!config.pjson.configFileBrowser && this.browserFile) {
    fs.unlinkSync(this.browserFile.fileName);
    this.browserFile = null;
  }
  if (!config.pjson.configFileNode && this.nodeFile) {
    fs.unlinkSync(this.nodeFile.fileName);
    this.nodeFile = null;
  }
