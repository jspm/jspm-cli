...
function JspmSystemConfig() {
  ...
  if (typeof this.nodeLibURL == 'string')
    this.nodeLibURL = path.resolve(isPlain(this.nodeLibURL) ? config.pjson.baseURL : config.pjson.dir, this.nodeLibURL);
  if (this.nodeRegistryPaths)
    Object.keys(this.nodeRegistryPaths).forEach(function(p) {
      var value = self.nodeRegistryPaths[p];
      self.nodeRegistryPaths[p] = path.resolve(isPlain(value) ? config.pjson.baseURL : config.pjson.dir, value);
    });
  ...
  if (this.browserBaseURL) {
    if (this.browserLibURL && isPlain(this.browserLibURL))
...
Object.defineProperty(JspmSystemConfig.prototype, 'nodePackagesURL', {
  get: function() {
    ...
    return this.nodeRegistryPaths && getPackagesURL(this.nodeRegistryPaths) || this._defaultNodePackagesURL;
  },
  set: function(packagesURL) {
    var self = this;
...
JspmSystemConfig.prototype.syncFile = function() {
  ...
  function setPathsConfig(name, browserPath, nodePath, orderLast) {
    // remove baseURL from browserPath and nodePath
    ...
    if (typeof nodePath == 'string' && nodePath.startsWith(config.pjson.baseURL))
