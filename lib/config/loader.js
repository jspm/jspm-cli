...
function LoaderConfig(fileName) {
  ...
  try {
    var packageConfigPaths = this.file.getProperties(['packageConfigPaths']);
  }
  catch(e) {}
...
LoaderConfig.prototype.prompt = function() {
  ...
  .then(function(baseURL) {
    ...
    return ui.input('Enter the module %format% of your package - `ESM` (ES2015 module), `CJS` or `AMD`', pjson.format || 'esm', {
    ...
    .then(function(format) {
      pjson.format = format;
      return ui.input('Enter the %main% entry point of your package within %' + (config.isDist ? pjson.dist : pjson.lib || pjson.dist) + '%', pjson.main || pjson.name + '.js');
    })
    .then(function(main) {
      pjson.main = main;
    });
