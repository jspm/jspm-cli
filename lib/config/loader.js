...
Config.prototype.read = function(prompts, sync) {
  ...
  .then(function(baseURL) {
    ...
    return ui.input('Which ES6 transpiler would you like to use, %Traceur%, %Babel% or %TypeScript%?', self.transpiler);
  })
  .then(function(transpiler) {
    transpiler = transpiler.toLowerCase();
    if (transpiler !== 'babel' && transpiler !== 'traceur' && transpiler !== 'typescript')
