...
function JspmSystemConfig(fileName) {
  ...
  if (this.browserBaseURL && this.browserBaseURL.endsWith('/'))
    this.browserBaseURL = this.browserBaseURL.substr(0, this.browserBaseURL.length - 1);
  ...
  function readAndSanitizePaths(pathsProp) {
    ...
    if (paths)
      Object.keys(paths).forEach(function(path) {
        ...
        if (value.startsWith(self.browserBaseURL + '/')) {
          value = value.substr(self.browserBaseURL.length + 1);
  ...
  if (this.browserLibURL && this.browserLibURL.endsWith('/'))
    this.browserLibURL = this.browserLibURL.substr(0, this.browserLibURL.length - 1);

  this.nodeLibURL = paths[this.packageName + '/'] || nodePaths[this.packageName + '/'];
  if (this.nodeLibURL && this.nodeLibURL.endsWith('/'))
    this.nodeLibURL = this.nodeLibURL.substr(0, this.nodeLibURL.length - 1);
...
JspmSystemConfig.prototype.syncFile = function() {
  ...
  function setPathsConfig(name, browserPath, nodePath, orderLast) {
    if (nodePath !== null && isPlain(browserPath) && browserPath == nodePath) {
