...
function initPrompts() {
  ...
  .then(function() {
    ...
    var defaultLibURL = (loaderBrowser.baseURL ? '' : '/') + path.relative(pjson.baseURL, pjson.lib);
  ...
  .then(function() {
    ...
    var defaultPackagesURL = (loaderBrowser.baseURL ? '' : '/') + path.relative(base, pjson.packages);
  ...
  .then(function() {
    ...
    return ui.input('%System.config local package main%', pjson.main || (loader.package || 'app' + '.js'), {
  ...
  // transpiler
  .then(function() {
    // NB typescript left out pending https://github.com/frankwallis/plugin-typescript/issues/81

    var transpilers = ['babel', 'traceur', 'none'];
    ...
    return ui.input('%System.config transpiler (Babel, Traceur, None)%', curTranspiler || 'babel', {
