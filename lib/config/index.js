...
exports.load = function(prompts, allowNoConfig) {
  ...
  .then(function() {
    ...
    if (config.loader.emptyConfig) {
      var upgradeConfig = new JspmSystemConfig(path.resolve(config.pjson.dir, 'config.js'));
  ...
  .then(function() {
    ...
    .then(function(doUpgrade) {
      ...
      oldConfigFile = config.loader.file.fileName;
      config.loader.file.fileName = config.pjson.configFile;
      if (oldConfigFile != config.pjson.configFile)
        config.loader.file.timestamp = -1;
      ...
      .then(function() {
        ...
        return require('../install').install(true, { force: true });
      })
      .then(function() {
        // delete the old config file
        if (oldConfigFile && oldConfigFile != config.pjson.configFile)
          return fs.unlink(oldConfigFile);
        // ui.log('info', 'Running project configuration...');
        // return require('./core').init(null, true);
      })
      .then(function() {
        ui.log('ok', 'jspm 0.17-alpha upgrade complete.\n\n' + 
          'Note that despite this upgrade path, there are still breaking changes in this release, so be sure to ' +
          'read the full changelog at https://github.com/jspm/jspm-cli/releases/tag/0.17.0-alpha.\n\n' +
          'Thanks for reporting further issues or feedback to help improve this release and for testing it out!\n');
      });
    });
  }))
  .then(function() {
...
function initPrompts() {
  ...
  .then(function() {
    ...
    // if we have a local package (directories.lib), then we set its loader
    // if we do not have a local package, then we set the transpiler
    // (we generally want to encourage the use of loader over transpiler for packages, although both are fine)
    ...
    return ui.input('%Which transpiler would you like to use (Babel, Traceur, TypeScript, None)%', curTranspiler || 'none', {
      options: transpilers
    })
  ...
  .then(function() {
    // because plugins are LOADERS
    // we only offer this when there is a local package
    // it may make sense to allow users to reconfigure the local package to enable??

    // we then set these BY FILETYPE
  })

  // Node core libs
  .then(function() {
    // enter a comma-separated list of Node modules to use in the project
    // (node-only become @node/x maps)
    // browser + node installs the browserify thangs
  });
}