...
function initPrompts() {
  ...
  .then(function() {
    ...
    return ui.input('%package.json configFiles folder%', (path.relative(base, configDir) || '.') + '/', {
  ...
  .then(function() {
    return ui.confirm('%Use package.json configFiles.jspm:dev?%', !!pjson.configFileDev, {
      info: 'Use a separate %jspm.dev.js% config file?\n\n' + 
          'This would be used for all %--dev% install configuration.'
    })
    .then(function(useDev) {
      if (useDev)
        pjson.configFileDev = pjson.configFileDev || path.resolve(path.dirname(pjson.configFile), 'jspm.dev.js');
      else
        pjson.configFileDev = null;
    });
  })
  ...
  .then(function() {
    if (!pjson.configFileBrowser && promptType == 'quick')
      return;

    return ui.confirm('%Use package.json configFiles.jspm:browser?%', !!pjson.configFileBrowser, {
    ...
    .then(function(useBrowser) {
      if (useBrowser)
        pjson.configFileBrowser = pjson.configFileBrowser || path.resolve(path.dirname(pjson.configFile), 'jspm.browser.js');
      else
        pjson.configFileBrowser = null;
    });
  })
  ...
  .then(function() {
    if (!pjson.configFileNode && promptType != 'custom')
      return;

    return ui.confirm('%Use package.json configFiles.jspm:node?%', !!pjson.configFileNode, {
      info: 'Use a separate %jspm.node.js% config file?\n\n' + 
          'This would be used for all %--node% install configuration.'
    })
    .then(function(useNode) {
      if (useNode)
        pjson.configFileNode = pjson.configFileNode || path.resolve(path.dirname(pjson.configFile), 'jspm.node.js');
      else
        pjson.configFileNode = null;
    });
  })
  ...
  .then(function() {
    ...
    return ui.input('%SystemJS.config browser baseURL' + (loader.package ? ' (optional)%' : '%'), loader.browserBaseURL || '/', {
      edit: true,
  ...
  .then(function() {
    ...
    var defaultLibPath = loader.nodeLibURL || (path.relative(pjson.baseURL, pjson.lib) || '.') + '/';
