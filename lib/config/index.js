...
var JspmDevConfig = require('./loader').JspmDevConfig;
...
exports.load = function(prompts, allowNoConfig) {
  ...
  .then(function() {
    ...
    config.loaderDev = new JspmDevConfig(config.pjson.configFileDev);
  })
  .then(function() {
    ...
    .then(function(doUpgrade) {
      ...
      .then(function() {
        ui.log('ok', 'jspm 0.17-beta upgrade complete.\n\n' +
          ...
          wordWrap('• There are now two new config files, %jspm.browser.js% and %jspm.dev.js% which must be included _before_ %jspm.config.js% in the browser.\n', process.stdout.columns - 4, 2, 0, true) + '\n' + 
...
exports.save = function() {
  if (config.allowNoConfig) {
    if (config.loader.file.changed)
      config.loader.write();
    if (config.loaderDev.file.changed)
      config.loaderDev.write();
  }
  else {
    config.loader.write();
    config.loaderDev.write();
...
function initPrompts() {
  ...
  .then(function() {
    return ui.input('%package.json directories.baseURL' + (loader.package ? ' (optional)%' : '%'), path.relative(base, pjson.baseURL) || '.', {
      ...
      validate: function(baseURL) {
        if (!baseURL && !loader.package)
          return 'The baseURL is only optional when a local package name (previously prompted) is used to alias local paths.';
    ...
    .then(function(baseURL) {
      // we note intent not to use baseURL, even though the baseURL still theoretically has a value
      if (!baseURL)
        useBaseURL = false;
  ...
  .then(function() {
    ...
    if (path.dirname(pjson.configFile) == path.dirname(pjson.configFileDev) &&
        path.basename(pjson.configFile) == 'jspm.config.js' && 
        path.basename(pjson.configFileDev) == 'jspm.dev.js')
    ...
    .then(function(configFolder) {
      ...
      pjson.configFileDev = path.resolve(base, configFolder, 'jspm.dev.js');
    });
  })
  ...
  .then(function() {
    ...
    return ui.input('%package.json configFiles.jspm:dev%', path.relative(base, pjson.configFileDev), {
      info: 'Enter a custom dev config file path.\n\nThis is the SystemJS config file for the %--dev% installs.'
    })
    .then(function(configFile) {
      pjson.configFileDev = path.resolve(base, configFile);
      config.loaderDev.file.rename(pjson.configFileDev);
    });
  })
  // baseURL in browser
  .then(function() {
    ...
    return ui.input('%System.config browser baseURL' + (loader.package ? ' (optional)%' : '%'), loader.browserBaseURL || '/', {
      edit: true,
      info: 'Enter the browser baseURL.\n\nThis must be the absolute URL matching the local %' + (path.relative(base, pjson.baseURL) || '.') + '% public folder served in the browser.',
      validate: function(baseURL) {
        if (!baseURL && !loader.package)
          return 'The baseURL is only optional when a local package name (previously prompted) is used to alias local paths.';
      }
    ...
    .then(function(baseURL) {
      // we can still opt-out of baseURL matching up to here
      if (!baseURL)
        useBaseURL = false;
      // when removing baseURL, alter lib and packages URLs to be absolute if not already
      if (!baseURL && loader.browserBaseURL) {
        if (loader.browserLibURL && !isPlain(loader.browserLibURL))
          loader.browserLibURL = loader.browserBaseURL + loader.browserLibURL;
        if (loader.browserPackagesURL && !isPlain(loader.browserPackagesURL))
          loader.browserPackagesURL = loader.browserBaseURL + loader.browserPackagesURL;
      }
  ...
  .then(function() {
    ...
    return ui.input('%System.config Node project path%', path.relative(base, pjson.lib) || '.', {
      ...
      validate: function(lib) {
        if (!inDir(path.resolve(base, lib) + '/', pjson.baseURL, false)) {
  ...
  .then(function() {
    ...
    var defaultLibURL = loader.browserLibURL || (loader.browserBaseURL ? '' : '/') + path.relative(pjson.baseURL, pjson.lib);

    if (promptType == 'quick' && useBaseURL) {
      loader.browserLibURL = defaultLibURL;
      return;
    }

    return ui.input('%System.config browser URL to ' + libPath + '%', loader.browserLibURL || defaultLibURL, {
      info: 'Enter the browser URL for the folder containing the local project code.\n\nThis should be the served %' + libPath + '% folder.' +
        (loader.browserBaseURL ? ' Leave out the leading %/% to set a baseURL-relative path.' : ''),
  ...
  .then(function() {
    ...
    var defaultPackagesURL = loader.browserPackagesURL || (loader.browserBaseURL ? '' : '/') + path.relative(base, pjson.packages);
    ...
    return ui.input('%SystemJS.config browser URL to ' + packagesPath + '%', loader.browserPackagesURL || defaultPackagesURL, {
