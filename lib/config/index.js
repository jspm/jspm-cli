...
exports.builderInit = false;
exports.load = function(prompts, builderInit) {
  if (config.loaded)
  ...
  .then(function() {
    config.builderInit = !!builderInit;
    ...
    if (!process.env.jspmConfigPath && !builderInit)
  ...
  .then(function(depsJSON) {
    ...
    if (!config.pjson.jspmAware && !builderInit || prompts)
      return initPrompts();
  })
  .then(function() {
    ...
    if (builderInit)
      throw new Error('The current project needs to be upgraded to jspm 0.17 before any build operations can run. Run %jspm init% to start the upgrade.');
...
exports.loadSync = function(builderInit) {
  if (config.loaded)
    return;
