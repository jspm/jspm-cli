...
exports.getLoaderConfig = function() {
  ...
  if (config.loader.package)
    cfg.paths[config.pjson.name + '/'] = cfg.paths[config.pjson.name + '/'] || config.pjson.lib + '/';
...
exports.load = function(prompts, allowNoConfig) {
  ...
  .then(function() {
    ...
    if (config.loader.emptyConfig) {
      var jspmConfig = new JspmSystemConfig(path.resolve(config.pjson.dir, 'jspm.js'));
      if (!jspmConfig.emptyConfig) {
        config.loader = jspmConfig;    
        fs.unlinkSync(config.loader.file.fileName);
