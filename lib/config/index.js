...
exports.getLoaderConfig = function() {
  ...
  if (config.loader.package && typeof cfg.paths[config.pjson.name + '/'] != 'string')
   cfg.paths[config.pjson.name + '/'] = toFileURL(config.pjson.lib + '/');
...
exports.load = function(prompts, allowNoConfig) {
  ...
  .then(function() {
    ...
    if (!allowNoConfig && config.loader.file.timestamp == -1)
