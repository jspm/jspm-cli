...
function initPrompts() {
  ...
  .then(function() {
    if (pjson.name && promptType != 'custom') {
      // we always set a baseURL by default
      // this also gives the baseURL error if configs loaded in wrong order
      if (!loaderBrowser.baseURL)
        loaderBrowser.setBaseURL('/');
      return;
    }

    return ui.input('%' + configPathBrowser + ' baseURL%', loaderBrowser.baseURL || '/', {
      edit: true,
