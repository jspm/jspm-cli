...
class ConfigFile {
    ...
    getProperties(memberArray, createIfUndefined = false) {
    ...
    getObject(memberArray = [], nested = true, createIfUndefined = false) {
    ...
    remove(memberArray, clearParentsIfMadeEmpty = false) {
        var parentProps = this.getProperties(memberArray.slice(0, memberArray.length - 1));
        if (!parentProps)
            return false;
        var self = this;
    ...
    setValue(memberArray, value, overwrite = true) {
    ...
    setProperties(memberArray, properties, clearIfEmpty = false, keepOrder = false, extend = true, overwrite = false) {
    ...
    setObject(memberArray = [], obj, clearIfEmpty = false, keepOrder = false) {
    ...
    extendObject(memberArray, obj, keepOrder = false) {
    ...
    prependObject(memberArray, obj, keepOrder = false) {
        if (!Array.isArray(memberArray)) {
            obj = memberArray;
            memberArray = [];
        }
    ...
    write() {
        ...
        if (timestamp !== this.timestamp)
            throw new common_1.JspmUserError('Configuration file ' + path.relative(process.cwd(), this.fileName) + ' has been modified by another process.');
...
function configError(memberArray, msg) {
    ...
    throw new common_1.JspmUserError(`Error reading ${common_1.bold(path.relative(process.cwd(), this.fileName))}\n\t` +
        (memberArray.length ? common_1.bold(memberArray.join('.')) + ' ' : 'File ') + msg + '.');
}
...
function setProperty(properties, key, value, ordering, overwrite = true) {
    var changed = false;
...
;
async function readJSONStyled(filePath) {
    try {
    ...
    try {
        ...
        throw new common_1.JspmError(`Error parsing JSON file ${filePath}.`);
    }
    return { json, style };
}
exports.readJSONStyled = readJSONStyled;
async function writeJSONStyled(filePath, json, style) {
    await new Promise((resolve, reject) => fs.writeFile(filePath, serializeJson(json, style), err => err ? reject(err) : resolve()));
}
exports.writeJSONStyled = writeJSONStyled;
...
function detectStyle(string) {
    let style = Object.assign({}, exports.defaultStyle);
...
function serializeJson(json, style) {
