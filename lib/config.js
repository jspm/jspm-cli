  ...
  .then(function(cfg) {
    ...
    for (var d in cfg.map) {
      if (typeof cfg.map[d] == 'string')
        ...
        config.depMap[d] = {};
        for (var m in depMap)
          config.depMap[m] = new Package(depMap[m]);
      }
    }
...
exports.save = function() {
  ...
  .then(function() {
    ...
    return asp(fs.writeFile)(config.configFile, 'System.config(' + JSON.stringify(config.curConfig, null, 2) + ');\n');
  })
...
function saveGlobalConfig() {
  try {
    ...
    if (e.code != 'EEXIST')
      cli.log('err', 'Unable to create jspm system folder\n' + e.stack);
  }
  try {
