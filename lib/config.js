...
exports.load = function(prompts) {
  ...
  .then(function() {
    return readJSON(path.resolve(config.pjson.packages, '.deps.json'));
  })
  .then(function(deps) {
    config.deps = deps;
    config.loaded = true;
...
exports.loadSync = function() {
  ...
  config.deps = JSON.parse(fs.readFileSync(path.resolve(config.pjson.packages, '.deps.json')));
...
exports.save = function() {
  ...
  .then(function() {
    return asp(fs.writeFile)(path.resolve(config.pjson.packages, '.deps.json'), JSON.stringify(config.deps, null, 2));
  })
  .then(function() {
