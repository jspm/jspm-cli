...
exports.load = function(prompts) {
  ...
  .then(function() {
    config.pjsonPath = process.env.jspmConfigPath || path.resolve(env.cwd(), 'package.json');
  ...
  .then(function(_prompts) {
    ...
    return ui.confirm('Configuration file %' + path.relative(env.cwd(), config.pjson.configFile) + '% doesn\'t exist, create it?', true)
    .then(function(create) {
      if (!create)
        throw 'Operation aborted.';
