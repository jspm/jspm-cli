  ...
  .catch(function(e) {
    ...
    if (e.toString().indexOf('globalName option') != -1)
      throw 'Build output to ' + opts.format + ' requires the global name to be set.\n\t\t\t' +
        'Try adding %--global-name ' + camelCase((expression.substr(expression.length - 3, 3) == '.js' ? expression.substr(0, expression.length - 3) : expression).split(/ |\//)[0]) + '%';
    else
      throw e;
  })
  .catch(function(e) {
...
function buildWatch(files, fn) {
  ...
  watcher.on('add', function(filepath) {
    ui.log('ok', 'File `' + filepath + '` created, rebuilding...');
    fn(filepath, true)
    .then(function() {
      watcher.close();
    });
  ...
  watcher.on('delete', function(filepath) {
    ui.log('warn', 'File `' + filepath + '` removed, cache invalidated pending rebuild...');
    fn(filepath, false);
  });
  watcher.on('change', function(filepath, root, stat) {
    ui.log('ok', 'File `' + filepath + '` changed, rebuilding...');
    fn(filepath, true)
    .then(function() {
      watcher.close();
    });
