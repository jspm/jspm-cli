...
Builder.prototype.bundle = function(expressionOrTree, outFile, opts) {
  ...
  .then(function(output) {
    ...
    if (opts.injectConfig) {
      config.loader.file.setValue(['bundles', output.bundleName], output.modules);
...
exports.depCache = function(expression) {
  ...
  .then(function(tree) {
    ...
    logTree(modules);
    config.loader.file.setObject(['depCache'], depCacheConfig);
  })
  .then(config.save)
  .then(function() {
...
function createWatcher(files, opts) {
  ...
  function change(filepath) {
    if (watchFiles.indexOf(filepath) == -1)
      return;
