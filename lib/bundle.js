...
exports.build = function(expression, fileName, opts) {
  ...
  .then(function(output) {
    logTree(output.modules, output.inlineMap);
    logBuild(path.relative(process.cwd(), fileName), opts);
  })
  .catch(function(e) {
...
function logTree(modules, inlineMap) {
  ...
  modules.sort().forEach(function(name) {
    ...
    if (inlineMap[name])
      inlineMap[name].forEach(function(inlineModule, index) {
        ui.log('info', '  `' + (index == inlineMap[name].length - 1 ? '└─ %' : '├─ %') + inlineModule + '%`');
      });
  });
  if (inlinedModules.length)
    ui.log('info', '\nModules in bold inlined via Rollup static optimizations.');
  ui.log('info', '');
}
