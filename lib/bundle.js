...
function extractOperations(args) {
  ...
  for (i = 1; i < args.length - 1; i += 2) {
    seq = chainPromise(args[i + 1], args[i]);
  }
  ...
  function chainPromise(name, operator) {
      return seq.then(function() {
        return expandGlob(name)
        .then(function(moduleNames) {
          pushOps(moduleNames, operator);
        });
      });
  }

  function pushOps(moduleNames, operator) {
...
function getModuleName(file) {
  ...
  for (var rule in paths) {
    if (paths.hasOwnProperty(rule)) {
      var resolvedPath = path.resolve(baseURL, paths[rule]).replace(/\\/g, '/');
      var pathDir = path.dirname(resolvedPath);
...
exports.bundle = function(moduleExpression, fileName, opts) {
  ...
  if (regex.test(moduleExpression)) {
    throw 'Bundle expression contains unsupported character or pattern';
  }
