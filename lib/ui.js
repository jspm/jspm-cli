...
var CtrlOption = function(options, message, def){
...
CtrlOption.prototype = {
  ...
  move: function() {
    ...
    len = input.length;
    for (var i = 0; i < len; i++)
      process.stdin.emit('keypress', input[i], { name: input[i] });

    this.def = this.options[this.cursol];
  },
};
...
exports.input = function(msg, def, options, queue) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (options.clearOnType && options.edit)
      ...
      // mode clearOnType and edit
      var ctrlOpt = new CtrlOption(options.options, questionMessage, def);
      inputFunc = function(key) {
        if (key.name === 'up') ctrlOpt.moveUp();
        if (key.name === 'down') ctrlOpt.moveDown();
      };
    }
