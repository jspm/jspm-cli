...
function isControlTerminateChar(c, control) {
  return !c || ('\r\n ,.' + (control == '_' ? '%' : '')).indexOf(c) != -1;
}

function highlight(text) {
  return text
    .replace(/([\s\`\(]|^)%([^%]+)%/g, function(match, index, capture) {
      if (isControlTerminateChar(text[index + 1], '%'))
    ...
    .replace(/([\s\`\%]|^)\_([^\_]+)\_/g, function(match, index, capture) {
      if (isControlTerminateChar(text[index + 1], '_'))
