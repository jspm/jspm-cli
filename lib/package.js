...
exports.download = function(pkg, options, installDeps) {
  ...
  var overrides = options.overrides ? extend({}, options.overrides) : {};
  ...
  downloading[pkg.exactName] = {
    preload: getPackageConfigPromise.then(function(pjson) {
      ...
      installDeps(depMap, extend(overrides, pjson.overrides || {}));
      return depMap;
    }),
    ...
    return (options.quick && dirInfo.hash ? Promise.resolve(true) : Promise.all([
      ...
      .then(function() {
        ...
        if (endpoint.getOverride) {
          ...
          return endpoint.getOverride(pkg.registry, pkg.package, pkg.version, override || config.pjson.overrides[pkg.exactName] || overrides[pkg.exactName]);
        }
