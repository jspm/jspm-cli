    ...
    return (options.quick && dirInfo.hash ? Promise.resolve(true) : Promise.all([
      ...
      .then(function() {
        ...
        if (registry.getOverride) {
          ...
          return registry.getOverride(pkg.endpoint, pkg.package, pkg.version, override || config.pjson.overrides[pkg.exactName]);
        }
...
function derivePackageConfig(pkg, pjson, override) {
  ...
  .then(function(pjson) {
    pjson.registry = pjson.registry || pkg.endpoint;
    return pjson;
  })
  .catch(function() {
