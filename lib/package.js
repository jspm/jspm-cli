  ...
  .then(function(dirExists) {
    ...
    .then(function(fresh) {
      ...
      .then(function() {
        ...
        return asp(fs.utimes)(path.resolve(downloadDir, '.jspm-hash'), +new Date(), +new Date());
      })
      .then(function() {
        return fresh;
      });
...
function derivePackageConfig(pkg, packageConfig, override) {
  ...
  .then(function(packageConfig) {
    ...
    if (pkg.registry == 'local' && !packageConfig.registry && packageConfig.dependencies && hasProperties(packageConfig.dependencies))
      throw '`' + pkg.exactName + '` needs the package.json %registry% property set for dependency installs to resolve. Try adding "registry": "jspm" to the package.json.';
...
function processPackage(pkg, dir, packageConfig, linked) {
  ...
  .then(function() {
    ...
    if (packageConfig.directories) {
      if (packageConfig.directories.dist) {
        distDir = packageConfig.directories.dist;
      }
      // NB deprecate
      else if (packageConfig.directories.lib && packageConfig.jspm && packageConfig.jspm.directories && packageConfig.jspm.directories.lib) {
        distDir = packageConfig.directories.lib;
        ui.log('warn', 'Package `' + pkg.exactName + '` has a %directories.lib% override configuration which will work, but is deprecated for %directories.dist% in future jspm versions.\n');
      }
    }
