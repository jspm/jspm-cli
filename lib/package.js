...
exports.lookup = function(pkg) {
  ...
  .then(function(lookup) {
    ...
    return function(version) {
      ...
      var match = pkg.copy().setVersion(lookupObj.version);
      match.hash = lookupObj.hash;
      return match;
    }
  });
}
...
function getVersionMatch(pkgVersion, versions) {
  ...
  for (var i = 0; i < branchVersions.length; i++) {
    if (semver.match(pkgVersion, branchVersions[i]))
  ...
  if (pkgVersion && versions[pkgVersion])
    return { version: pkgVersion, hash: versions[pkgVersion] };
}
...
function doPreload(pjson, preload) {
  ...
  preload(processDeps(pjson.dependencies, pjson.registry));
}
...
exports.download = function(pkg, options, preload) {
  ...
  .then(function(fresh) {
    ...
    if (endpoint.getPackageConfig && preload)
