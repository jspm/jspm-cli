...
exports.download = function(pkg, options, depsCallback) {
  ...
  function doDepsCallback(depRanges) {
    sentDeps.forEach(function(sentDep) {
      delete depRanges.peerDeps[sentDep];
      delete depRanges.deps[sentDep];
    });
    sentDeps = sentDeps.concat(Object.keys(depRanges.deps)).concat(Object.keys(depRanges.peerDeps));
    depsCallback(depRanges);
  }
  ...
  .then(function(dirExists) {
    ...
    .then(function(fresh) {
      ...
      .then(function(cacheFresh) {
        ...
        .then(function() {
          return asp(fs.writeFile)(path.resolve(cacheDir, '.jspm-hash'), fullHash + newLine + cfgHash);
        });
      })
