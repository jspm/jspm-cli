...
exports.locate = function(target) {
  ...
  .then(function(located) {
    ...
    if (located.notfound)
      throw new TypeError('Repo `' + target.name + '` not found!');
...
exports.lookup = function(pkg) {
  ...
  .then(function(lookup) {
    if (lookup.notfound)
      throw new TypeError('Repo `' + pkg.package + '` not found!');
...
// options.inject, options.override
// note if it is a symlink, we leave it unaltered

var downloading = {};
exports.download = function(pkg, options, preload) {
  ...
  return downloading[pkg.exactName].promise = options.inject ? _pkg.getCDNPackageJSON(pkg).then(getPackageConfigResolve, getPackageConfigReject) : Promise.resolve()
  .then(function() {
    ...
    .then(function() {
      ...
      return isFresh(downloadDir, fullHash);
    })
    .then(function(_fresh) {
      ...
      return Promise.resolve(config.force ? false : isFresh(cacheDir, fullHash))
      ...
      .then(function() {
        if (!pjson)
          throw 'no pjson for ' + pkg.exactName;
