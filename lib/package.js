...
function getOverride(pkg, manualOverride) {
  ...
  .then(function() {
    ...
    if ((alreadyInstalled || config.pjson.overrides[pkg.exactName]) && !manualOverride)
      return Promise.resolve(config.pjson.overrides[pkg.exactName] || {});
...
exports.download = function(pkg, options, installDeps, installPeerDeps) {
  ...
  .then(function(dirExists) {
    ...
    .then(function(pkgHash) {
      ...
      fullHash = hash + md5(JSON.stringify(override)) + endpoint.versionString + 'jspm@.' + jspmVersion;
    ...
    .then(function(fresh) {
      ...
      .then(function(cacheFresh) {
        ...
        getPackageConfigPromise.suppressUnhandledRejections();
