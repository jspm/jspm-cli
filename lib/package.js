...
exports.download = function(pkg, options, installDeps) {
  ...
  if (downloading[pkg.exactName]) {
    downloading[pkg.exactName].preload.then(function(depMap) {
      installDeps(depMap, pkg.exactName);
      return depMap;
    });
  ...
  downloading[pkg.exactName] = {
    preload: getPackageConfigPromise.then(function(pjson) {
      ...
      // dont install mapped dependencies
    ...
    return (options.quick && dirInfo.hash ? Promise.resolve(true) : Promise.all([
      ...
      .then(function() {
        ...
        if (endpoint.getOverride) {
          if (!override) {
            for (var overrideName in config.pjson.overrides) {
              if (overrideName.startsWith(pkg.name + '@')) {
                var overrideVersion = '^' + overrideName.split('@').pop();
                // use an override if it is compatible per semver
                if (semver.match(overrideVersion, pkg.version)) {
                  override = config.pjson.overrides[overrideName];
