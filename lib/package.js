...
exports.locate = function(target) {
  if (!target.endpoint) {
    target = new Package(target.exactName);
  ...
  return Promise.resolve(endpoint.locate(target.package))
  .then(function(located) {
    // NB support versioned registry
    if (target.endpoint == config.globalConfig.registry)
...
exports.download = function(pkg, jspmPackages, options, preload) {
  ...
  .then(function() {
    ...
    var registry = ep.load(config.globalConfig.registry);
  ...
  .then(function(_override) {
    ...
    .then(function(fresh) {
      ...
      .then(function(fresh) {
        ...
        .then(function() {
          return _pkg.readJSON(path.resolve(downloadDir, '.jspm.json'));
        })
  ...
  .then(function(fresh) {
    ...
    if (endpoint.getPackageConfig)
      ...
      .then(function(_pjson) {
        ...
        if (!pjson)
          pjson = derivePackageConfig(_pjson, override);
        doPreload(pjson, preload);
        return fresh;
      });
    return fresh;
