...
exports.download = function(pkg, jspmPackages, options, preload) {
  ...
  if (options.inject)
    return _pkg.getCDNPackageJSON(pkg.exactName).then(function(pjson) {
      // NB this should pjson.jspm when CDN updates to this mode?
    ...
    .then(function(fresh) {
      ...
      .then(function(fresh) {
        ...
        .then(function(_pjson) {
          pjson = _pjson
          return _pkg.createMain(pkg, pjson.jspm, downloadDir);
        })
        .then(function() {
          return true;
        });
  ...
  .then(function(fresh) {
    ...
    .then(function(_pjson) {
      ...
      if (_pjson)
        pjson = derivePackageConfig(pjson = _pjson, override || {});

      // if still no package.json, load from the downloaded folder
      if (!pjson)
        return Promise.resolve(_pkg.readJSON(path.resolve(dir, 'package.json')))
        .then(function(_pjson) {
          if (_pjson)
            derivePackageConfig(pjson = _pjson, overrides || {});
        });
  ...
  .then(function() {
    var depMap = {};
    return processDeps(pjson.jspm.dependencies, pjson.jspm.registry);
  });
}
