...
exports.locate = function(target) {
  ...
  if (_pkg.registryCache[target.exactName])
    return _pkg.registryCache[target.exactName];

  cli.log('info', 'Looking up %' + target.name + '% in registry');
...
exports.lookup = function(pkg) {
  return new Promise(function(resolve, reject) {
    if (versionCache[pkg.package])
      return resolve(versionCache[pkg.package]);
  ...
  .then(function(versions) {
    ...
    function createLookup(version) {
      if (!versions[version])
        throw 'No version match found for `' + pkg.exactName + '`';
      var lookup = new Package(pkg.name + '@' + version);
      lookup.hash = versions[version];
      return lookup;
    }

    versionCache[pkg.package] = versions;
    ...
    if (pkg.version && (!rangeMatch || rangeMatch[4]))
      return createLookup(rangeVersion);
    
    // create version list of only stable semvers
    var versionList = [];
    for (var v in versions) {
      ...
      // only check against stable versions
      if (!semverMatch || !semverMatch[3] || !semverMatch[1] || semverMatch[4])
        continue;

      versions[v] = versions[v] || hash;
      versionList.push(v);
    }
    ...
    for (var i = versionList.length - 1; i >=0; i--)
      if (!pkg.version || semver.match(pkg.version, versionList[i]))
        return createLookup(versionList[i]);

    // if we asked for latest, and nothing stable found, use master
    if (!pkg.version)
      return createLookup('master');
...
exports.inject = function(pkg, override) {
  ...
  // NB when the below download code is on CDN, this doesn't need to do all of this,
  //    and can use CDN package.json directly
  Promise.all([
    _pkg.checkOverride(pkg.exactName, true), 
    override ? Promise.resolve(override) : _pkg.checkOverride(pkg.exactName)
  ])
  .then(function(pjsons) {
    var pjson = pjsons[0] || {};
    var override = pjsons[1];
...
exports.download = function(pkg, jspmPackages, override, force) {
  ...
  .then(function(fresh) {
    ...
    return Promise.all([
      ...
      .then(function() {
        return new Promise(function(resolve, reject) {
          var promise = _pkg.loadEndpoint(pkg.endpoint).download(pkg.package, pkg.version, pkg.hash, downloadDir, resolve, reject);
          if (promise)
            promise.then(resolve, reject);
        });
      })
    ...
    .then(function(pjsons) {
      var pjson = pjsons && pjsons[0] || {};
      var override = pjsons && pjsons[1];
    ...
    .then(function(pjson) {
      return build.buildPackage(downloadDir, pjson)
      .then(function() {
        return pjson;
      });
    })
