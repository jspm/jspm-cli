      ...
      registryInstance[hook] = function() {
        ...
        return new Promise(function(resolve, reject) {
          ...
          function tryHook() {
            ...
            (self.queue || Promise.resolve(self))
          ...
          function checkConfigure(err) {
            if (err && err.config) {
              ...
              // also only print warning for first such error, if multiple arrive in a batch
              if (!self.queue) {
                var suffix = (typeof args[0] === 'string' ?
                    ' for `' + registry + ':' + args[0] + '`' :
                    '');
                ui.log('warn', 'Error on ' + hook + suffix + '\n\n' + err.message);
                ui.log('warn', 'Configuration may resolve the issue');

                self.queue = exports.configure(registry)
                  .then(function() {
                    // replace registered instance
                    delete registryClasses[registry];
                    return exports.load(registry);
                  });
              }
