...
function registryRequire(name, baseDir) {
  var resolved;
  try {
...
exports.load = function(registry) {
  ...
  try {
    ...
    var RegistryClass = registryRequire(options.handler, process.env.jspmConfigPath && path.dirname(process.env.jspmConfigPath) || path.resolve(process.cwd()));
    var registryPackageJSON = registryRequire(options.handler + '/package.json', process.env.jspmConfigPath && path.dirname(process.env.jspmConfigPath) || path.resolve(process.cwd()));
...
exports.configure = function(registry) {
  ...
  try {
    RegistryClass = registryRequire(handler, process.env.jspmConfigPath && path.dirname(process.env.jspmConfigPath) + '/' || path.resolve(process.cwd()));
  }
  catch(e) {
