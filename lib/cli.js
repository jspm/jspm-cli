...
`);
            ...
            case 'bin':
                {
                    let options;
                    ({ args, options } = opts_1.readOptions(args, ['path', 'cmd']));
                    if (options.path && options.cmd)
                        throw new common_1.JspmUserError(`--path is incompatible with --cmd in ${common_1.bold('jspm bin')} command.`);
                    ...
                    if (options.path) {
                        ...
                        if (args.length === 0) {
                            ...
                            if (args.length > 1)
                                throw new common_1.JspmUserError(`${common_1.bold('jspm bin')} only supports a single script name.`);
            ...
            case 'trace':
                {
                    let options;
                    ({ args, options } = opts_1.readOptions(args, ['react-native', 'production', 'electron', 'node', 'deps'], ['out', 'in']));
                    project = new api.Project(projectPath, { offline, preferOffline, userInput, cli: true });
            ...
            default:
                ...
                throw new common_1.JspmUserError(`Command or file ${common_1.bold(cmd)} does not exist.`);
        }
    }
    catch (err) {
