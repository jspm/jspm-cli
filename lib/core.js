...
var install = exports.install = function(name, target, options) {
  ...
  .then(function() {
    ...
    // we do this before downloading so downloads don't block the process
    return pkg.loadConfig(lookup, options.override, options.inject)
    .then(function(pjson) {
      ...
      // in parallel download and start kicking off dependency installs
      return Promise.all([
        depMap ? install(depMap, {
          force: options.force,
        ...
        options.inject ? Promise.resolve() : pkg.download(lookup, pjson, config.jspmPackages, options.force)
      ]);
    });
  })
