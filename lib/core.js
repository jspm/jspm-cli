...
exports.bundle = function(moduleName, fileName) {
  var firstTree, secondTree, moduleName, fileName, operator, moduleToOperateOn, verboseOperator, args = arguments[0];

  if (args.length > 5) {
      return Promise.resolve(ui.log('warn', 'Arithmetic only supports a single operation'));
  } else if (args.length > 3) {
      if (args[4].indexOf('.js') === -1) {
        return Promise.resolve(ui.log('warn', 'The last argument must be a valid js filename'));
      }
      moduleName = args[1];
      operator = args[2];
      moduleToOperateOn = args[3];
      fileName = args[4];
      ...
      .then(function() {
        ...
        setLocalPaths();
        return systemBuilder.trace(moduleName, config.main)
        .then(function(trace) {
            firstTree = trace.tree;
            return systemBuilder.trace(moduleToOperateOn);
        })
        .then(function(trace) {
            secondTree = trace.tree;
            return verboseOperator(firstTree, secondTree);
        })
        .then(function(tree) {
