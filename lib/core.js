...
function deprecate(name, versionRange) {
  ...
  var versionList = config.loader.versions[name] || [];
...
// removes from config.loader.baseMap, pruning orphaned dependencies
// also removes from config.pjson.dependencies
function removePackage(name) {
  ...
  var removeName = config.loader.baseMap[name].name;
  var removeVersion = config.loader.baseMap[name].version;

  // now remove it
  delete config.loader.baseMap[name];
...
/* 
  config.loader.baseMap = {
    jquery: new PackageName('npm:thing@1.2')
  };
  config.loader.depMap = {
    'npm:thing@1.2.1': {
      dep: new PackageName('github:some/dep@2.3')
    }
  };
  config.loader.versions = {
...
function getCriticalRanges(name, version) {
  ...
  for (var d in config.loader.depMap) {
    var curDepMap = config.loader.depMap[d];
  ...
  for (var n in config.loader.baseMap) {
    var module = config.loader.baseMap[n];
    if (module.name != name)
      continue;
    if (ranges.indexOf(module.version) == -1)
      ranges.push(module.version);
...
var install = exports.install = function(name, target, options) {
  ...
  .then(function(located) {
    ...
    // ensure that the located endpoint is in our loader endpoints config
    config.loader.ensureEndpoint(located.endpoint);

    if (options.primary)
      return;

    var versionList = config.loader.versions[target.name];
    if (!versionList)
      return;
    ...
    if (!options.force)
      for (var i = versionList.length - 1; i >= 0; i--) {
        ...
        var _lookup = new PackageName(target.exactName);
  ...
  .then(function(_lookup) {
    ...
    oldDep = config.pjson.dependencies[name];
    oldMap = config.loader.baseMap[name];
    oldVersions = oldMap && config.loader.versions[oldMap.name];
  ...
  .then(function(deprecated) {
    ...
    var versionList = config.loader.versions[lookup.name] || [];
    ...
    check: for (var i = 0; i < versionList.length; i++) {
      ...
      if (lookup.version != curVersion) {
        ...
        delete config.loader.depMap[oldName];
        versionList.splice(i--, 1);
      }
    }
    ...
    if (!useExisting)
      ...
      /* return ui.confirm('`' + lookup.name + '` already has version' + (versionList.length > 1 ? 's `' : ' `') + versionList.join('`, `') + 
  ...
  // then write in our new config.loader.baseMap and config.pjson.dependencies
  .then(function() {
    ...
    .then(function() {
      ...
      if (!oldDep.endpoint && !initialTarget.endpoint && oldDep.name == initialTarget.name) {
        ...
        oldDep = new PackageName(target.name);
        oldDep.setVersion(v);
      }
      ...
      return ui.confirm('%' + name + '% installed as `' + oldDep.exactName + '`, are you sure you want to install this to `' + target.exactName + '`?', true)
      .then(function(confirm) {
        if (!confirm)
          throw 'Operation cancelled';
      })
    ...
    .then(function() {
      ...
      return ui.confirm('%' + name + '% is configured to `' + oldMap.exactName + '`, are you sure you want to change this to `' + target.exactName + '`?', true)
      .then(function(confirm) {
        if (!confirm)
          throw 'Operation cancelled';
      });
    ...
    .then(function() {
      ...
      var depMap = config.loader.depMap[options.parent] = config.loader.depMap[options.parent] || {};
    ...
    .then(function() {
      // add the version
      var versionList = config.loader.versions[lookup.name] = config.loader.versions[lookup.name] || [];
      ...
      if (options.primary) {
        ...
        config.loader.baseMap[name] = target;
      }
    })
  ...
  .then(function() {
    if (options.link)
      return link.symlink(lookup, config.pjson.packages, options);
    ...
    .then(function() {
      ...
      return downloads[lookup.exactName] = pkg.download(lookup, config.pjson.packages, options, function preload(deps) {
        // preload!
    ...
    .then(function(depMap) {
      ...
      if (!options.force && depMap && config.loader.depMap[lookup.exactName]) {
        ...
        var curDepMap = {};
        for (var d in config.loader.depMap[lookup.exactName]) {
          curDepMap[d] = config.loader.depMap[lookup.exactName][d].exactName;
        }
...
exports.dlLoader = function(unminified, edge) {
  ...
  return config.load()
  .then(function() {
    ui.log('info', 'Downloading loader files to %' + path.relative(process.pwd(), config.pjson.packages) + '%');
  ...
  .then(function() {
    return Promise.all([
      pkg.lookup(new PackageName('github:ModuleLoader/es6-module-loader@' + (!edge ? loaderVersions[0] : 'master')))
      ...
      pkg.lookup(new PackageName('github:systemjs/systemjs@' + (!edge ? loaderVersions[1] : 'master')))
      .then(function(lookup) {
        var downloadFiles = {};
      ...
      pkg.lookup(new PackageName('github:jmcriffey/bower-traceur@' + (!edge ? loaderVersions[2] : 'master')))
      .then(function(lookup) {
        var downloadFiles = {};
