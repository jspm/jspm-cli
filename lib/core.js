...
// if so, it does
// supports custom parserNames -> '6to5', 'traceur' (default)
exports.checkDlLoader = function() {
  var parserName;
  return config.load()
  .then(function() {
    parserName = config.loader.parser
  ...
  .then(function(cacheVersions) {
    if (cacheVersions.toString() != [lVersions.esml, lVersions.system, parserName == '6to5' ? '' : lVersions.traceur, parserName == '6to5' ? lVersions['6to5'] : ''].join(','))
      return exports.dlLoader(parserName);
...
exports.dlLoader = function(parserName, unminified, edge) {
  ...
  .then(function() {
    ...
    ui.log('info', '  `' + parserName + '@' + using[parserName] + '`');

    return asp(fs.writeFile)(path.resolve(config.pjson.packages, '.loaderversions'), 
        [lVersions.esml, lVersions.system, parserName == '6to5' ? '' : lVersions.traceur, parserName == '6to5' ? lVersions['6to5'] : ''].join(','));
  })
  .then(function() {
    if (config.loader.parser != parserName) {
      config.loader.parser = parserName;
      return config.save()
      .then(function() {
        ui.log('ok', 'ES6 transpiler set to %' + parserName + '%.');
      });
    }
  })
  .then(function() {
