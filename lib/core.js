    ...
    tPkgs.forEach(function(p) {
      if (config.loader.baseMap[p])
        if (config.loader.baseMap[p].version !== tPackages[p].split('@').pop() && !nodeSemver.satisfies(config.loader.baseMap[p].version, tPackages[p].split('@').pop()))
          ui.log('warn', '`' + p + '@' + config.loader.baseMap[p].version + '` is unsupported for this version of jspm. Use %jspm dl-loader% to update.');
    });
  })
  .then(function() {
...
exports.dlLoader = function(transpilerName, unminified, edge, latest, updateTranspiler) {
