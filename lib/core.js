...
var install = exports.install = function(name, target, options) {
  ...
  .then(function() {
    ...
    .then(function() {
      ...
      return cli.confirm('`' + options.parent + '` currently has dependency %' + name + '% set to `' + depMap[name].exactName + '`, but the upgrade expects `' + target.exactName + '`. Update?', true)
      .then(function(confirm) {
        if (confirm)
          depMap[name] = target;
      });
...
exports.bundle = function(moduleName, fileName) {
  ...
  .then(config.save)
  .then(function() {
    cli.log('ok', 'Built into `' + fileName + '`');
  })
  .catch(function(e) {
    cli.log('err', e.stack || e);
  });
