...
var install = exports.install = function(name, target, options) {
  ...
  .then(function(deprecated) {
    ...
    if (!useExisting)
      cli.log('info', '%Forking% `' + lookup.name + '` to use versions `' + versionList.concat([lookup.version]).join('`, `') + '`');
  ...
  .then(function() {
    ...
    .then(function() {
      ...
      return cli.confirm('`' + parent + '` currently has dependency %' + name + '% set to `' + depMap[name].exactName + '`, but the upgrade expects `' + target.exactName + '`. Update?', true)
      .then(function(confirm) {
        if (confirm)
          depMap[name] = target;
      });
  ...
  .then(function() {
    ...
    if (options.primary)
      ...
      cli.log('ok', (lookup.fresh ? 'Up to date - `' : '`') + target.exactName + '` (' + lookup.version + ')');
  });
}
...
exports.depCache = function(moduleName) {
  ...
  .then(function() {
    ...
    if (!moduleName)
      return cli.input('No main entry point is provided, please specify the module to trace', '~/app').then(function(main) {
        config.main = main;
      });
  })
  .then(function() {
    cli.log('info', 'Injecting the traced dependency tree for `' + moduleName + '`...');
    setLocalPaths();
  ...
  .then(config.save)
  .then(function() {
    cli.log('ok', 'Depenency tree injected');
...
exports.bundle = function(moduleName, fileName) {
  return config.load()
  .then(function() {
    config.main = moduleName || config.main;
    if (!config.main)
      return cli.input('No main entry point is provided, please specify the module to build', 'app/main').then(function(main) {
        config.main = main;
      });
  })
  .then(function() {
    cli.log('info', 'Building the single-file dependency tree for `' + config.main + '`...');
    setLocalPaths();
    return systemBuilder.build(config.main, config.curConfig, 'build.js')
  }) 
  .then(function() {
  ...
  .then(config.save)
  .then(function() {
    cli.log('ok', 'Built into `build.js`');
  })
  .catch(function(e) {
    cli.log('err', e.stack || e);
  });
...
exports.build = function() {
  ...
  .then(function() {
    ...
    if (pjson.buildConfig.transpileES6 === undefined) {
      savePjson = true;
      return cli.input('Transpile ES6?', true);
    }
  })
    ...
    for (var dep in config.depMap) {
      ...
      for (var subdep in config.depMap[dep])
        if ((deprecated = deprecate(subdep.name, subdep.version)).length)
          cli.log('info', 'Deprecated `' + deprecated.join('`, `') + '`');
    }
  })
  .then(config.save)
  .then(function() {
