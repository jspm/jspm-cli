...
exports.normalize = function(moduleName, parentName) {
  config.loadSync();
  var loader = new System.constructor();
  loader.config(config.getLoaderConfig());
...
exports.init = function init(basePath) {
  ...
  .then(function() {
    ...
    ui.log('ok', 'package.json at %' + path.relative(process.cwd(), config.pjsonPath) + '%\n' + 
        ...
        (config.pjson.configFileDev ? ' and %' + path.relative(process.cwd(), config.pjson.configFileDev) + '%' : '') +
        (config.pjson.configFileBrowser ? ' and %' + path.relative(process.cwd(), config.pjson.configFileBrowser) + '%' : '') +
        (config.pjson.configFileNode ? ' and %' + path.relative(process.cwd(), config.pjson.configFileNode) + '%' : ''));
  })
  .then(function() {
