  ...
  .then(function() {
    ...
    tPkgs.forEach(function(p) {
      if (config.loader.baseMap[p])
        if (!nodeSemver.satisfies(config.loader.baseMap[p].version, tPackages[p].split('@')[1]))
          ui.log('warn', '`' + p + '@' + config.loader.baseMap[p].version + '` is out of date. Use %jspm dl-loader% to update.');
    });
  })
  .then(function() {
...
exports.dlLoader = function(transpilerName, unminified, edge, updateTranspiler) {
  ...
  .then(function() {
    return dl('systemjs', 'github:systemjs/systemjs', !edge ? systemVersion : 'master')
    .then(function(version) {
      ...
      return Promise.all([
        ...
        cp('systemjs/dist/polyfills' + min + '.js', 'polyfills.js'),
        unminified || cp('systemjs/dist/polyfills.src.js', 'polyfills.src.js'),
        unminified || cp('systemjs/dist/polyfills.js.map', 'polyfills.js.map')
      ]);
    });
  })
  .then(function() {
  ...
  .then(function() {
    return exports.dlTranspiler(transpilerName, updateTranspiler);
  })
  .then(function() {
