...
var install = exports.install = function(name, target, options) {
  ...
  .then(function() {
    ...
    if (installs[lookup.exactName]) {
      // options.seen provides support for circular tree dependencies
      if (options.seen && options.seen.indexOf(lookup.exactName) != -1)
        return;
      return installs[lookup.exactName];
    }

    options.seen = options.seen && options.seen.concat([]) || [];
    options.seen.push(lookup.exactName);

    return installs[lookup.exactName] = Promise.resolve()
    .then(function() {
      if (options.link)
        return link.symlink(lookup, config.jspmPackages, options);

      return pkg.download(lookup, config.jspmPackages, options, function preload(deps) {
        // preload!
...
exports.dlLoader = function(unminified, edge) {
  ...
  .then(function(lookup) {
    ...
    downloadFiles['/ModuleLoader/es6-module-loader/' + (edge ? '' : 'v') + lookup.version + '/dist/es6-module-loader' + min + '.js'] = 'es6-module-loader' + (!edge ? '@' + loaderVersions[0] : '') + '.js';
  ...
  .then(function(lookup) {
    ...
    downloadFiles['/systemjs/systemjs/' + lookup.version + '/dist/system' + min + '.js'] = 'system' + (!edge ? '@' + loaderVersions[1] : '') + '.js';
    return pkg.lookup(new Package('github:jmcriffey/bower-traceur@' + (!edge ? loaderVersions[2] : 'master')));
  })
  .then(function(lookup) {
    usedVersions[2] = lookup.version;
    downloadFiles['/jmcriffey/bower-traceur/' + lookup.version + '/traceur' + min + '.js'] = 'traceur' + (!edge ? '@' + loaderVersions[2] : '') + '.js';
    downloadFiles['/jmcriffey/bower-traceur-runtime/' + lookup.version + '/traceur-runtime' + min + '.js'] = 'traceur-runtime' + (!edge ? '@' + loaderVersions[2] : '') + '.js';
  })
  .then(function() {
    // delete old versions
    return asp(fs.readdir)(config.jspmPackages)
