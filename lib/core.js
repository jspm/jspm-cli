...
exports.init = function init(basePath, ask) {
  ...
  var relBase = path.relative(env.cwd(), path.dirname(process.env.jspmConfigPath));
  if (relBase !== '')
  ...
  .then(function() {
    ui.log('ok', 'Verified package.json at %' + path.relative(env.cwd(), config.pjsonPath) + '%\nVerified config file at %' + path.relative(env.cwd(), config.pjson.configFile) + '%');
  })
  .then(function() {
