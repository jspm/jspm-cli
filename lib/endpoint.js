...
exports.load = function(endpoint) {
  ...
  try {
    ...
    endpintInstance.constructor = endpointClass;
          ...
          function checkRetry(err) {
            ...
            if (retries <= maxRetries) {
              ui.log('warn', (err ? 'Error' : 'Timed out') + ' on ' + hook + ' '
                 + (typeof args[0] == 'string' ? 'for `' + endpoint + ':' + args[0] + '`' : '') + ', retrying (' + retries + ').'
                 + (err ? '\n' + (err.stack || err) : ''));
              return tryHook();
            }
            else
              return reject('Reached maximum number of retries.');
          }
