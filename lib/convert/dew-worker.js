...
function transformDew(ast, source, resolveMap, deps) {
    const { code: dewTransform } = babel.transformFromAst(ast, source, {
        ...
        plugins: [[dewTransformPlugin, {
                    ...
                    esmDependencies: dew_resolve_1.isESM,
...
async function tryCreateDew(filePath, pkgBasePath, files, folderMains, deps) {
    ...
    try {
        ...
        for (const require of requires)
            resolveMap[require] = dew_resolve_1.relativeResolve(require, filePath, pkgBasePath, files, folderMains, deps);
...
module.exports = function convert(dir, files, folderMains, namedExports, deps, callback) {
    return Promise.resolve()
        ...
        for (const file of Object.keys(files).sort()) {
            ...
            if (file.endsWith(('.json'))) {
                ...
                conversionPromises.push(tryCreateDew(path.resolve(dir, file), dir, files, folderMains, deps).then(err => {
                    if (err)
                        return;
