...
function convertCJSConfig(pcfg) {
    ...
    if (pcfg.map) {
        ...
        for (const match of Object.keys(pcfg.map)) {
            ...
            if (dew_resolve_1.isESM(match))
                continue;
            newMap[isRel ? dew_resolve_1.toDew(match) : dew_resolve_1.toDewPlain(match, deps)] = convertMappingToDew(mapping, isRel ? undefined : deps);
        }
    }
    if (pcfg.bin) {
        let newBin;
        for (const bin of Object.keys(pcfg.bin)) {
            ...
            newBin[bin] = dew_resolve_1.toDew(target);
        }
        if (newBin)
            pcfg.bin = newBin;
    }
...
function convertMappingToDew(mapping, deps) {
    if (typeof mapping === 'string')
        return deps ? dew_resolve_1.toDewPlain(mapping, deps) : dew_resolve_1.toDew(mapping);
    const newMap = {};
    for (const condition of Object.keys(mapping))
        newMap[condition] = convertMappingToDew(mapping[condition], deps);
    return newMap;
}
/*
 * Conversion process:
...
async function convertCJSPackage(log, dir, pkgName, pcfg) {
    ...
    await new Promise((resolve, reject) => {
        convertWorker(dir, convertFiles, folderMains, pcfg.namedExports, dew_resolve_1.pcfgToDeps(pcfg), (err, convertedFiles) => {
            if (err)
    ...
    if (pcfg.main) {
        ...
        if (!(pcfg.map && pcfg.map[pcfg.main])) {
            let resolved = dew_resolve_1.resolveFile(pcfg.main, convertFiles);
            // JSON mains
            if (resolved) {
