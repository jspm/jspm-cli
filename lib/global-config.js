...
if (HOME !== process.env.USERPROFILE && !fs.existsSync(path.join(HOME, '.jspm'))) {
  var OLD_HOME = process.env.USERPROFILE;
  ui.log('info', 'Migrating global jspm folder from `' + OLD_HOME + '` to `' + HOME + '`, please wait...');
  ...
  try {
    ...
    ui.log('info', 'Moving linked packages...');
    fs.mkdirSync(path.resolve(to, 'linked'));

    function recurseAndCopySymlinks(dir) {
      var files = fs.readdirSync(dir);
      files.forEach(function(file) {
        var linkTo;
        var stat = fs.statSync(path.resolve(dir, file));
        if (stat.isDirectory()) {
          recurseAndCopySymlinks(path.resolve(dir, file));
        }
        else {
          linkTo = fs.readlinkSync(path.resolve(dir, file));
          fs.symlinkSync(path.resolve(dir.replace(from, to), file), linkTo, 'junction');
        }
      });
    }
    recurseAndCopySymlinks(path.resolve(from, 'linked'));

    ui.log('info', 'Clearing files in `' + OLD_HOME + '`...');
  ...
  ui.log('ok', 'Migration successful.');
}
