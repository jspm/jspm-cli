...
function getDefaultTarget(env, target, esm) {
    ...
    if (env.node === false || env.browser === true) {
        envTargetName = 'browser ' + (target && target.join(', ') || envTarget.browsers || 'esmodules baseline');
    }
    else {
...
async function build(input, opts) {
    ...
    try {
        ...
        if (err.frame) {
            throw new common_1.JspmUserError(err.frame);
        }
        if (err.loc) {
    ...
    if (opts.out) {
        ...
        if (opts.log)
            ui_1.ok(`Built into ${common_1.bold(opts.out)}${envTargetName ? ' for ' + envTargetName : ''}`);
    }
    else {
        chunks = build.chunks;
        if (opts.removeDir) {
