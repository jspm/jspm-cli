  ...
  .then(function() {
    ...
    return Promise.all(Object.keys(targets).map(function(name) {
      ...
      if (!(target instanceof PackageName)) {
        // install jquery@1.2.3 / npm:jquery / github:components/jquery@1.2.3
      ...
      return install(name, target, options);
    }))
    .then(function() {
      return saveInstall();
    });
...
/*
 * install('jquery', 'jquery', { latest, lock, parent, inject, link, override } [, seen])
 *
 * Install modes:
...
function install(name, target, options, seen) {
  ...
  .then(function(located) {
    ...
    // determine if we've already got a resolution for this name and parent
    if (options.parent)
      existing = installed.depMap[options.parent] && installed.depMap[options.parent][name];
    else
      existing = installed.baseMap[name];

    // lock if necessary
    if (options.lock && existing)
  ...
  .then(function(satisfied) {
    ...
    .then(function(getLatestMatch) {
      ...
      if (!resolution)
        throw new Error('No version match found for `' + target.exactName + '`');
      ...
      return loadExistingForkRanges(resolution, options.inject)
      .then(function() {
        ...
        resolveForks(installed, name, options.parent, resolution, function(forkVersion, forkRanges) {
          ...
          if (options.parent && !consolidated && !options.latest) {
        ...
        if (!options.parent) {
          ...
          if (!secondaryRanges[options.parent][name])
            ...
            assert(secondaryRanges[options.parent][name].exactName == target.exactName, 'secondary range clash ' + options.parent + ', ' + secondaryRanges[options.parent][name].exactName + ', ' + target.exactName);
        }
  ...
  .then(function() {
    ...
    return (options.inject ? pkg.inject(resolution, downloadDeps) : pkg.download(resolution, options.override, downloadDeps))
    .then(function(fresh) {
...
function loadExistingForkRanges(resolution, inject) {
  var tree = installed;
...
exports.uninstall = function(name) {
  ...
  .then(function() {
    installed = installed || config.loader;
    if (!config.pjson.dependencies[name] && !installed.baseMap[name]) {
