...
function inspectVersions(forks) {
  var list = getVersionsList();
  var versions = list.versions;
  var linkedVersions = list.linkedVersions;

  return Object.keys(versions).map(function(dep) {
    var newestVersion = versions[dep].sort(semver.compare)[0];
    var haveLinked = !!(linkedVersions[dep + '@' + newestVersion]);
...
exports.inspectVersions = inspectVersions;
