...
function visitForkRanges(tree, resolution, name, parentName, visit) {
  ...
  Object.keys(tree.depMap).forEach(function(parent) {
    ...
    Object.keys(curDepMap)
      .filter(function(dep) {
        return matchesSecondaryDepName(dep) && isNotForkOfSelf(dep) && hasRange(dep);
      })
      .forEach(function(dep) {
        var ranges = secondaryRanges[parent];
        var deps = ranges && ranges.deps && ranges.deps[dep];
        var secondary = curDepMap[dep];

        visit(dep, parent, secondary, deps);
      });


    function matchesSecondaryDepName(dep) {
      return curDepMap[dep].name === resolution.name;
    }
    function isNotForkOfSelf(dep) {
      return dep !== name || parent !== parentName;
    }
    function hasRange(dep) {
      var ranges = secondaryRanges[parent];
      return ranges && ranges.deps && ranges.deps[dep];
    }
  });
}
