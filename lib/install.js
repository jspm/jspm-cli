...
function _clean() {
  ...
  .then(function() {
    ...
    // 4. remove dependencies on .dependencies.json not being used
...
function clean() {
  return new Promise(function(resolve, reject) {
    fs.lstat(config.pjson.packages, function(err, stats) {
      if (err) {
        reject(err);
      } else {
        resolve(stats.isSymbolicLink() ? true : _clean());
      }
    });
  });
}
