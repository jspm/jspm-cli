...
function install(name, target, options, seen) {
  ...
  .then(function(located) {
    ...
    // lock if necessary
    if (options.lock && (resolution = options.parent ? installed.depMap[options.parent][name] : installed.baseMap[name]))
...
exports.resolveOnly = function(pkg) {
  ...
  if (!pkg.version || !pkg.endpoint)
    return ui.log('warn', 'Resolve --only must take an exact package of the form `endpoint:pkg@version`.');
