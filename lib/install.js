...
exports.install = function(targets, options) {
  ...
  .then(function() {
    ...
    var k, defaultTargets = {};
    for (k in config.pjson.devDependencies)
      defaultTargets[k] = config.pjson.devDependencies[k];
    for (k in config.pjson.dependencies)
      defaultTargets[k] = config.pjson.dependencies[k];
    ...
    if (targets === true)
      ...
      Object.keys(targets).forEach(function(module) {
        ...
        var targetIsVersion = target.indexOf(':') === -1;
        var configured = defaultTargets[module];
        if (!configured || targetIsVersion) {
          var moduleExpr = targetIsVersion ? module + '@' + target : module;
