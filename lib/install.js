...
exports.install = function(targets, options) {
  ...
  .then(function() {
    ...
    return Promise.all(Object.keys(targets).map(function(name) {
      ...
      if ((bulk || options.update) && !config.pjson.dependencies[name]) {
...
function install(name, target, options, seen) {
  ...
  .then(function(located) {
    ...
    .then(function(targetRange) {
      ...
      target = config.pjson.dependencies[name] || config.pjson.peerDependencies[name] || config.pjson.devDependencies[name] || target;
      return existing;
    });
  })
  ...
  .then(function(alreadyInstalled) {
  ...
  function storeResolution() {
    ...
    if (!options.parent || options.peer) {
      ...
      // peer dependencies of a dev parent install save as devDependencies primaries of the parent
      if (options.parent && options.peer && options.dev)
        options.peer = false;
      // if a peer, but primary install exists, we save as a primary install
      else if (options.parent && options.peer && config.pjson.dependencies[name])
        options.peer = false;
      else if (!options.parent && !options.peer && !options.dev && config.pjson.peerDependencies[name])
        options.peer = true;
  ...
  .then(function(acceptedRange) {
    ...
    return ui.confirm('Would you like to install %' + name + '% to the new install range?', true, {
      info: 'Peer dependency conflict for %' + name + '%:\n' +
        (acceptedRange ? 
          '  Currently resolving to `' + acceptedRange.exactName + '`.\n'
    ...
    .then(function(useNew) {
      ...
      return ui.confirm('Would you like to retain the current resolution?', true)
