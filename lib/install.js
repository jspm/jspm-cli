...
/*
 * install('jquery', 'jquery', { latest, lock, parent, inject, link } [, seen])
 *
 * Install modes:
...
function loadExistingForkRanges(resolution, options) {
  var tree = installed;
  return Promise.all(Object.keys(tree.baseMap).map(function(dep) {
    ...
    return loadExistingRange(dep, null, options);
  }))
  .then(function() {
    return Promise.all(Object.keys(tree.depMap).map(function(parent) {
      ...
      return Promise.all(Object.keys(curDepMap).map(function(dep) {
        ...
        return loadExistingRange(dep, parent, options);
      }));
    }));
  });
}
...
function loadExistingRange(name, parent, options) {
      ...
      .then(function() {
        ...
        return secondaryDepsPromises[parent] = new Promise(function(resolve, reject) {
          fs.exists(pjsonPath, function(exists) {
            if (exists)
              return resolve();
            pkg.download(parentPkg, { lock: options.lock, latest: options.latest, inject: options.inject }, resolve).catch(reject);
          });
        })
        .then(function(depMap) {
          if (depMap)
...
function clean() {
  ...
  .then(function(packageDirs) {
    return Promise.all(packageDirs
    .filter(function(dir) {
