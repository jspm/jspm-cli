...
function install(name, target, options, seen) {
  ...
  .then(function(located) {
    ...
    .then(function(targetRange) {
      ...
      target = config.pjson.peerDependencies[name] || config.pjson.dependencies[name] || config.pjson.devDependencies[name] || target;
      return existing;
    });
  })
...
function visitForkRanges(tree, resolution, name, parentName, visit) {
  ...
  Object.keys(tree.depMap).forEach(function(parent) {
    ...
    Object.keys(curDepMap).forEach(function(dep) {
      ...
      visit(dep, parent, secondary, ranges[dep].deps);
    });
  });
}
...
function clean(hard) {
  ...
  .then(function() {
    if (hard) {
      ...
      config.loaderBrowser.file.changed = true;
      config.loaderDev.file.changed = true;
    }
