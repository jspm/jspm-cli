        ...
        for (const scopeName of Object.keys(map.scopes)) {
            ...
            if (common_1.isURL(scopeName, true)) {
                ...
                for (const pkgName of Object.keys(scope)) {
                    ...
                    let rebased = common_1.isURL(pkg, true) ? pkg : path.relative(resolvedScope, path.resolve(resolvedScope, pkg)).replace(/\\/g, '/');
                    if (!rebased.startsWith('../'))
                        rebased = './' + rebased;
                    if (pkg.endsWith('/'))
                        rebased += '/';
                ...
                if (absolute) {
                    if (rebasedScope.startsWith('../'))
                        throw new common_1.JspmUserError(`Unable to reference scope ${scopeName} at ${newScope}. The base for the import map must a higher path than its mappings.`);
    ...
    for (const scope of Object.keys(importMap.scopes)) {
        ...
        for (const pkgName of Object.keys(imports)) {
            ...
            if (existing && existing !== newTargetResolved)
                throw new common_1.JspmUserError(`Cannot flatten scopes due to conflict for ${common_1.bold(pkgName)} between ${existing} and ${newTargetResolved}.`);
