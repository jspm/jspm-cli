...
async function tryCreateDew(filePath, pkgBasePath, files, folderMains) {
    ...
    try {
        ...
        for (const require of requires)
            resolveMap[require] = common_1.toDew(relative_resolve_1.relativeResolve(require, filePath, pkgBasePath, files, folderMains), true);
        ...
        for (const file of Object.keys(files).sort()) {
            ...
            if (file.endsWith(('.json'))) {
                ...
                conversionPromises.push(createJsonDew(path.resolve(dir, file)).then(err => {
                    ...
                    esmWrapperPromises.push(writeESMWrapper(dir, file + '.js', common_1.toDew(path.basename(file), false), namedExports));
                }));
            }
            else {
                ...
                conversionPromises.push(tryCreateDew(path.resolve(dir, file), dir, files, folderMains).then(err => {
                    ...
                    esmWrapperPromises.push(writeESMWrapper(dir, file, common_1.toDew(path.basename(file), false), namedExports));
                }));
            }
        }
