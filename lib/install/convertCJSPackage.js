...
function convertCJSConfig(pcfg) {
    ...
    if (pcfg.main) {
        if (pcfg.map && pcfg.map['./' + pcfg.main]) {
            ...
            newMap['./' + common_1.toDew(pcfg.main, false)] = newMap['./index.dew.js'] = convertMappingToDew(pcfg.map['./' + pcfg.main], false);
        }
    }
    else {
        pcfg.main = 'index.js';
    ...
    if (pcfg.map) {
        for (const match of Object.keys(pcfg.map)) {
            ...
            newMap[common_1.toDew(match, !isRel)] = convertMappingToDew(mapping, !isRel);
        }
    }
    if (pcfg.bin) {
        let newBin;
        for (const bin of Object.keys(pcfg.bin)) {
            ...
            newBin[bin] = common_1.toDew(target, false);
        }
        if (newBin)
            pcfg.bin = newBin;
    }
...
function convertMappingToDew(mapping, checkMain) {
    if (typeof mapping === 'string')
        return common_1.toDew(mapping, checkMain);
    const newMap = {};
...
async function convertCJSPackage(log, dir, pkgName, pcfg) {
    ...
    for (const folderPath of Object.keys(folderMains)) {
        ...
        if (resolved.endsWith('.node'))
            ...
            writeDewAlias(folderPath + '.dew.js', common_1.toDew(resolved.substr(folderPath.lastIndexOf('/') + 1), false));
    }
    // main main
    if (pcfg.main && pcfg.main !== 'index' && pcfg.main !== 'index.js' && pcfg.main !== 'index.json') {
        ...
        if (resolved.endsWith('.node'))
            ...
            writeDewAlias('index.dew.js', common_1.toDew(resolved, false));
    }
    else {
