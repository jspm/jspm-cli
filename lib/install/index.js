...
class Installer {
    ...
    */
    ...
    async packageInstall(install) {
        if (install.type === package_1.DepType.peer) {
            // this.project.log.warn('Peer resolution yet to be implemented.');
            install.type = package_1.DepType.primary;
        }
        ...
        return this.installs[installId] = (async () => {
            ...
            let override = install.override && package_1.processPackageConfig(install.override, true);
            let source;
            let resolvedPkg;
    ...
    async resourceInstall(install) {
        ...
        return this.installs[installId] = (async () => {
            ...
            if (isLink) {
                ...
                config = package_1.processPackageConfig(json || {}, true);
                if (override) {
    ...
    async readCheckedOutConfig(resolvedPkgName, override, linked = false) {
        ...
        let config = package_1.processPackageConfig(json || {}, true);
        if (override) {
            ({ config, override } = package_1.overridePackageConfig(config, override));
            if (override)
