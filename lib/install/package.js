...
function validateOverride(pcfg, name) {
    for (let p in pcfg) {
        if (validKeys.indexOf(p) === -1)
            throw new common_1.JspmUserError(`${common_1.bold(`"${p}"`)} is not a valid property to override for ${common_1.highlight(name)}.`);
    }
    return true;
}
exports.validateOverride = validateOverride;
function processPackageConfig(pcfg, partial = false, registry = '') {
    const processed = processPjsonConfig(pcfg);
    if (processed.peerDependencies)
        delete processed.peerDependencies;
    if (processed.devDependencies)
...
function processPackageTarget(depName, depTarget, defaultRegistry = '', rangeConversion = false) {
    ...
    if (targetNameLen > 2)
        throw new common_1.JspmUserError(`Invalid package target name ${common_1.bold(name)}`);
