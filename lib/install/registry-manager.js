...
class RegistryManager {
  ...
  This may be from a previous jspm version and can be removed with ${common_1.bold(`jspm config --unset registries.${name}`)}.`);
    ...
    async auth(url, credentials, unauthorized = false) {
    ...
    async resolve(pkg, override, edge = false) {
        ...
        if (resolved.override) {
            resolvedOverride = package_1.processPackageConfig(resolved.override, true);
            if (override)
    ...
    async ensureInstall(source, override, verificationFailure, fullVerification = false) {
        ...
        if (config) {
            config = package_1.processPackageConfig(config, true);
            if (override) {
        ...
        try {
            ...
            if (!pjson)
                pjson = {};
            if (!config) {
                let pjsonConfig = package_1.processPackageConfig(pjson, true);
                const serializedConfig = package_1.serializePackageConfig(pjsonConfig);
                if (override)
