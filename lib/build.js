...
exports.filterIgnoreAndFiles = function(dir, ignore, files) {
  ...
  .then(function(allFiles) {
    ...
    // map removal functions
    var funcs = removeFiles.map(function(removeFile) {
    ...
    funcs.push(new Promise(function(resolve, reject){
      //get remaining files in the output dir
      asp(glob)(dir+"/**/*").then(function(remainingFiles){
        //check remaining files (loop in reverse to remove subdirectories first)
        for (var i = remainingFiles.length-1, l=0; i >= l; i--) {
          var file = remainingFiles[i];
          //check if it is a directory and if it is empty
          if(fs.lstatSync(file).isDirectory() && !fs.readdirSync(file).length){
            fs.rmdirSync(file);
          }
        }
        resolve();
