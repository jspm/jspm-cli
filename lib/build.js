...
exports.filterIgnoreAndFiles = function(dir, ignore, files) {
  ...
  .then(function(allFiles) {
    ...
    var resolvedRemoveFiles = removeFiles.map(function(removeFile) {
      return path.resolve(dir, removeFile);
    });

    var symlinks = [];

    return Promise.all(resolvedRemoveFiles.map(function(removeFile) {
      return asp(fs.lstat)(removeFile)
      .then(function(stats) {
        if (stats.isSymbolicLink())
          symlinks.push(removeFile);
      });
    }))
    .then(function() {
      return resolvedRemoveFiles.filter(function(removeFile) {
        // if a file is in a symlinked directory
        // then remove it from the list
        return !symlinks.some(function(symlinkFile) {
          return inDir(removeFile, symlinkFile + '/');
        });
      })
