...
exports.link = function(dir, name, options) {
  ...
  // create the symlink
  .then(function() {
    if (pjson.directories && pjson.directories.lib && !pjson.directories.dist)
      ui.log('warn', 'Linked package.json specifies directories.lib, which is deprecated in jspm 0.17.\n' +
          'If this is the intended folder to link from the package, set it as directories.dist instead.\n');
    var libDir = pjson.directories && pjson.directories.dist;
    if (libDir)
    ...
    .then(function(hasPackageJson) {
      ...
      return ui.confirm('Create a package.json symlink in the ' + path.relative(dir, libDir) + ' folder?', true, {
        info: 'In order to link the directories.dist folder %' + path.relative(dir, libDir) + '%, ' + 
      ...
      .then(function(confirm) {
        ...
        return asp(fs.symlink)(path.relative(libDir, path.join(dir, 'package.json')), path.resolve(libDir, 'package.json'), 'junction');
      });
    })
    .then(function() {
      dir = libDir;
