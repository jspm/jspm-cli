...
exports.link = function(name, dir, force) {
  ...
  .then(function(_pjson) {
    ...
    var packageName = pkg.name || _pjson.jspm && _pjson.jspm.name || _pjson.name;
    var endpoint = pkg.registry || _pjson.jspm && _pjson.jspm.registry || _pjson.registry;
    var version = pkg.version || _pjson.jspm && _pjson.jspm.version || _pjson.version;

    if (!packageName)
      throw 'Package name not specified.';
    if (!endpoint)
      throw 'Package registry endpoint not specified.';
    if (!version)
      throw 'Package version not specified.';

    pkg = new PackageName(endpoint + ':' + packageName + '@' + version);

    pjson = package.derivePackageConfig(pkg, _pjson);
  })
  .then(function() {
