...
exports.link = function(name, dir, force) {
  var pjson, baseLinkDir, linkDir, pkg;
  ...
  .then(function() {
    baseLinkDir = linkDir = path.resolve(config.HOME, '.jspm', 'linked', pkg.endpoint, pkg.exactPackage);
    try {
      if (fs.existsSync(baseLinkDir)) {
  ...
  .then(function(abort) {
    ...
    return asp(rimraf)(baseLinkDir)
    // create it
    .then(function() {
      if (pjson.directories) {
        if (pjson.directories.dist) {
          linkDir = path.resolve(baseLinkDir, pjson.directories.dist);
          dir = path.resolve(dir, pjson.directories.dist);
        } 
        else if (pjson.directories.lib) {
          linkDir = path.resolve(baseLinkDir, pjson.directories.lib);
    ...
    .then(function() {
      return asp(fs.writeFile)(path.resolve(baseLinkDir, '.jspm.json'), JSON.stringify(pjson, null, 2));
    });
  })
