...
exports.link = function(dir, name, options) {
  ...
  .then(function() {
    ...
    .then(function(pjson) {
      if (!pkg.package && !pjson.name)
        throw 'A target package name to link must be provided or set in the package.json.\nTry running %jspm link ' + dir + ' ' + dir.split('/').pop() + '%.';
  ...
  .then(function() {
    return install.install(pkg.package, pkg.exactName, { lock: true, quick: options.quick, force: options.force, link: true });
  })
  .then(function(aborted) {
    if (!aborted)
      ui.log('info', 'Run %jspm install ' + pkg.exactName + '% to relink the package configuration file.');
    else
