...
function throwFieldMissing (fieldName: string) {
  throw new JspmUserError(`Package configuration must specify a ${bold(fieldName)} field to publish.`);
}
function throwFieldInvalid (value: string, expected: string) {
...
export function pack (project: Project, files: void | string[], ignore: void | string[]) {
  ...
  if (pjson.ignore instanceof Array === false) {
    ...
    try {
      const npmignore = await new Promise((resolve, reject) => 
        fs.readFile(path.join(project.projectPath, '.npmignore'), (err, source) => err ? reject(err) : resolve(source))
      );
      ignoreIsFile = true;
      ...
      try {
        const gitignore = await new Promise((resolve, reject) =>
          fs.readFile(path.join(project.projectPath, '.gitignore'), (err, source) => err ? reject(err) : resolve(source))
        );
        ignoreIsFile = true;
